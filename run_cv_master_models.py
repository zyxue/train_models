...
if master_model_name == 'all':
    ...
    df_trva = DF_TRVA.query('disease in {0}'.format(diseases_with_sufficient_normal))
    df_test = DF_TEST.query('disease in {0}'.format(diseases_with_sufficient_normal))
...
df_cv = cv.gen_df_cv_master(grid_search_result)
df_cv.to_csv(
